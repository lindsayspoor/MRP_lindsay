...
def plot_dynamic_on_static(path_plot,success_rates_1_1_2, success_rates_1_1_3, success_rates_MWPM5,error_rates_eval):
    

    plt.figure(figsize=(7,6))
    #for j in range(success_rates.shape[0]):
    plt.grid()
    plt.plot(error_rates_eval, success_rates_MWPM5, label=f'd=5 MWPM', linestyle='-.',linewidth=1.1, color='black')
    plt.scatter(error_rates_eval, success_rates_1_1_2, label=r"$(N,N_{new},k)=(1,1,2)$", marker="^", s=45, color = 'blue', edgecolors='black',zorder=10)
    plt.scatter(error_rates_eval, success_rates_1_1_3, label=r"$(N,N_{new},k)=(1,1,3)$", marker="o", s=35, color = 'darkorange', edgecolors='black',zorder=11)
    #plt.scatter(error_rates_eval, success_rates_2_2_3, label=r"$(N,N_{new},k)=(2,2,3)$", marker="H", s=35, color = 'yellow', edgecolors='black', zorder=12)
    #plt.scatter(error_rates_eval, success_rates_2_2_4, label=r"$(N,N_{new},k)=(2,2,4)$", marker="s", s=35, color = 'green', edgecolors='black', zorder=13)
    #plt.plot(error_rates_eval, success_rates_all_MWPM_30, label=f'd=30 MWPM', linestyle='--',linewidth=0.7, color='black')
    plt.xlabel(r'$p_{error}$', fontsize=16)
    plt.xlim((0.005,error_rates_eval[-1]+0.005))
    plt.xticks(fontsize=15)
...
def plot_multiple_boxes_dynamic(path_plot, moves_agent1,moves_agent2, moves_agent3, moves_agent4):

    labels = ['Random',r'lr=0.001', r'lr=0.0001','annealing']
    moves_all=[moves_agent1,moves_agent3, moves_agent2, moves_agent4]
